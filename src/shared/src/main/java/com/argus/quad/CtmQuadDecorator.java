...
import com.argus.verify.VerifyRecorder;
...
/**
 * Argus's default {@link QuadDecorator}: consults the
 ...
 * <h2>Verify mode</h2>
 *
 * <p>When {@code argus.verify_mode} is enabled (via the
 ...
 * for everything; Argus's selection is recorded) - the
...
public final class CtmQuadDecorator implements QuadDecorator {
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger("argus/ctm-decorator");
    ...
    public Optional<QuadRef> decorate(QuadRef quad, QuadContext context) {
        ...
        try {
            ...
            LOGGER.debug(
                    "[argus] CTM selector failed for {}@{} face {}: {}",
                    context.blockId(),
    ...
    private static Optional<QuadRef> resolveSwap(
        ...
        try {
            ...
            // debug.
            LOGGER.debug(
                    "[argus] withSprite failed for sprite {}: {}",
