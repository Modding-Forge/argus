...
/**
 * Sodium meshing adapter for Argus's Better Grass mode.
 *
 ...
 */
public final class ArgusSodiumBetterGrass {
    ...
    /**
     * Applies Argus's Better-Grass-owned snow side cover to full solid side
    ...
    private static boolean hasFakeSnowCoverAbove(BlockAndTintGetter level,
        ...
        return isSnowDirtFamily(state)
                && ArgusSodiumBetterSnow.shouldRenderLayer(level, above,
                abovePos);
    }
    ...
    private static void logMultilayerFallback() {
        if (!ArgusConfigHolder.get().ctmDebugLogging()) {
            return;
        }
