...
uniform samplerBuffer ArgusCtmOverlayTint;
...
in vec4 lightColor;
in vec2 texCoord0;
flat in uint argusCtmPayload;
flat in vec4 argusCtmSourceUv;
flat in uvec4 argusCtmOverlayMaterials;
flat in vec4 argusCtmMaterialUv;
flat in int argusCtmVertexId;

out vec4 fragColor;

vec2 argusNormalizeCtmUv(vec2 uv, uint payload) {
...
uint argusOverlayMaterialId(uint rawMaterial) {
    return rawMaterial & 0xFFFFu;
}

uint argusOverlayOrientation(uint rawMaterial) {
    return (rawMaterial >> 16) & 0x7u;
}

vec2 argusOrientOverlayUv(vec2 uv, uint orientation) {
    if (orientation == 1u) {
...
float argusOverlayShade(vec3 shadeRgb) {
...
float argusOverlayAlpha(float alpha) {
...
void main() {
    ...
    bool argusOverlay = false;
    if (argusCtmPayload != 0u) {
        vec2 sourceSize = argusCtmSourceUv.zw - argusCtmSourceUv.xy;
        localUv = clamp((texCoord0 - argusCtmSourceUv.xy)
                / max(sourceSize, vec2(0.000001)), vec2(0.0), vec2(1.0));
        localUv = argusNormalizeCtmUv(localUv, argusCtmPayload);
        sampleUv = mix(argusCtmMaterialUv.xy, argusCtmMaterialUv.zw, localUv);
        argusOverlay = ((argusCtmPayload >> 25) & 0x1u) != 0u;
    }
    vec4 sampled = UseRgss == 1
            ? sampleRGSS(Sampler0, sampleUv, 1.0f / TextureSize)
            : sampleNearest(Sampler0, sampleUv, 1.0f / TextureSize);
    bool argusPrelit = false;
    if (argusOverlay) {
        vec4 base = UseRgss == 1
        ...
        argusPrelit = true;
        for (int i = 0; i < 4; ++i) {
            uint rawMaterial = argusCtmOverlayMaterials[i];
            uint material = argusOverlayMaterialId(rawMaterial);
            if (material == 0u) {
                continue;
            }
            vec4 materialUv = texelFetch(ArgusCtmMaterials, int(material));
            vec2 overlayLocalUv = argusOrientOverlayUv(
                    localUv, argusOverlayOrientation(rawMaterial));
            ...
            vec4 overlayTint = texelFetch(
                    ArgusCtmOverlayTint, argusCtmVertexId * 4 + i);
            float overlayAlpha = argusOverlayAlpha(
                    overlaySample.a * overlayTint.a);
            vec4 overlay = vec4(
                    overlaySample.rgb * overlayTint.rgb
                            * argusOverlayShade(shadeColor.rgb)
                            * lightColor.rgb,
                    overlayAlpha);
            sampled = mix(sampled, vec4(overlay.rgb, 1.0), overlay.a);
        }
    }
    vec4 color = argusPrelit ? sampled : sampled * vertexColor;
#ifdef ARGUS_CTM_DEBUG_OVERLAY
    if (argusCtmPayload != 0u) {
