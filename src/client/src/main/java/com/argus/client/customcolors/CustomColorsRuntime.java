...
public final class CustomColorsRuntime {
    ...
    private static final MethodHandle BIOME_CLIMATE_SETTINGS =
            biomeClimateSettingsAccessor();
    ...
    private static boolean biomeActive() {
        return active() && ArgusConfigHolder.get().biomeColorsEnabled();
    }

    private static boolean skyActive() {
        return active() && ArgusConfigHolder.get().skyColorsEnabled();
    }
    ...
    private static int sampleRule(CustomColorsClientSnapshot.RuntimeRule rule,
        ...
        if (rule.rule().format() == com.argus.customcolors.ColormapFormat.GRID) {
    ...
    private static int sampleBiome(Biome biome,
                                   ColormapImage image) {
        Biome.ClimateSettings climate = climateSettings(biome);
        return image.sampleVanilla(climate.temperature(),
                climate.downfall());
    }

    private static Biome.ClimateSettings climateSettings(Biome biome) {
        try {
